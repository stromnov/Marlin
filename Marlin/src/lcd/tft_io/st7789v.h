...
static const uint16_t st7789v_init[] = {
  ...
  ESC_REG(ST7789V_MADCTL), 0x00B0, //ST7789V_MADCTL_DATA,
  ESC_REG(ST7789V_COLMOD), 0x0055,

  ESC_REG(ST7789V_NORON),
  ESC_REG(ST7789V_DISPON),

  //ESC_REG(ST7789V_SWRESET), ESC_DELAY(5000),
  // ESC_REG(ST7789V_SLPOUT), ESC_DELAY(100),

  // ESC_REG(ST7789V_PORCTRL), 0x0005, 0x0005, 0x0000, 0x0033, 0x0033,//b2
  // ESC_REG(ST7789V_GCTRL), 0x0035, //b7
  // ESC_REG(ST7789V_VCOMS), 0x0028, //bb
  // ESC_REG(ST7789V_LCMCTRL), 0x002C,
  // ESC_REG(ST7789V_VDVVRHEN), 0x0001,// 0x00C3, //c2
  // ESC_REG(ST7789V_VRHS), 0x000b,
  // ESC_REG(ST7789V_VDVS), 0x0020,  //c4
  // ESC_REG(ST7789V_FRCTRL2), 0x000F, //c6
  // ESC_REG(ST7789V_PWCTRL1), 0x00A4, 0x00A1, //d0
  // ESC_REG(ST7789V_PVGAMCTRL),0xd0,0x01,0x08,0x0f,0x11,0x2a,0x36,0x55,0x44,0x3a,0x0b,0x06,0x11,0x20, //e0

  // ESC_REG(ST7789V_MADCTL), 0x00b0, //ST7789V_MADCTL_DATA,
  // ESC_REG(ST7789V_COLMOD), 0x0005,
  // ESC_REG(ST7789V_NVGAMCTRL), 0xd0,0x02,0x07,0x0a,0x0b,0x18,0x34,0x43,0x4a,0x2b,0x1b,0x1c,0x22,0x1f,
  // ESC_REG(ST7789V_NORON),
  // ESC_REG(ST7789V_DISPON),
